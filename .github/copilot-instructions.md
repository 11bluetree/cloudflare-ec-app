...
- **テスト作成方針**: `docs/testing-guide.md` を参照

## コーディングルール

- 一貫したコードスタイルを保つため、`prettier`と`eslint`を使用
- TypeScriptの型安全性を最大限に活用
- ユースケース・e2e、エンティティなどは先にテストを書く（TDD推奨）
  - その後に実装コードを書いてテストが通るまで繰り返す
...
- アーキテクチャ: `docs/architecture.md` - レイヤードアーキテクチャとディレクトリ構造の詳細
- 完全な要件: `docs/prd.md`
- データモデル: `docs/prd.md` セクション4.1
- API仕様: `docs/prd.md` セクション5
- テスト方針: `docs/testing-guide.md` - テストケース名やテストデータの作成ルール
