...
- ✅ 1商品あたり**1〜5個**のオプション定義が必要（例: 色、サイズ、容量、素材、香り）
- ✅ 1オプションあたり**50個以下**の値
- ✅ **オプション組み合わせ総数は100以下**である必要がある（`<= 100`）
  - 例: 色3種×サイズ3種×容量2種 = 18通り ✅ OK
  - 例: 色10種×サイズ10種×容量2種 = 100通り ✅ OK（上限）
  - 例: 色50種×サイズ50種 = 2,500通り ❌ NG（100を超える）
- ✅ `option_name`は**50文字以内**
- ✅ `value`は**50文字以内**

### オプションのデフォルト

- ✅ **すべての商品には最低1つのオプション定義が必須**
- 単一バリアント商品（オプションなし商品）の場合は、**デフォルトオプション**を定義する
  - オプション名: `title`
  - オプション値: `default`
  - 例: 「タイトル:デフォルト」
...
この値が100を超える場合、オプション値の追加を制限する。

### バリアント作成時の制約

- バリアントのオプションは、商品レベルで定義されたオプションから**のみ**選択可能
- 未定義のオプション名・値は使用不可
- バリアント作成時に、選択されたオプションの組み合わせが**100通り以下**であることを検証
