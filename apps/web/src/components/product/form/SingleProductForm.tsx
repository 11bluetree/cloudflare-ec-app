 ...
 */
export const SingleProductForm: React.FC<SingleProductFormProps> = ({ register, errors, status }) => {
  const isPublished = status === 'published';

  return (
    <div className="space-y-6">
      <FormField label="SKU" required={isPublished} error={errors.variants?.[0]?.sku?.message} htmlFor="variant-sku">
        <Input
          id="variant-sku"
      ...
      {/* バリアント配列全体のエラーメッセージ */}
      {errors.variants?.root?.message && <p className="mt-2 text-sm text-red-600">{errors.variants.root.message}</p>}
