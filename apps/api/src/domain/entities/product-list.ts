...
const productListItemSchema = z
  .object({
    ...
    variants: z
      ...
      .min(MIN_VARIANTS_PER_PRODUCT, { message: '商品には最低1つのバリアントが必要です' }),
  })
...
const productListSchema = z.object({
  items: z
    .array(z.custom<ProductListItem>())
    .max(MAX_PRODUCTS_PER_PAGE, { message: `1ページあたりの商品数は${MAX_PRODUCTS_PER_PAGE}件以下である必要があります` }),
});
