...
export const categories = sqliteTable('categories', {
  id: text('id', { length: 26 }).primaryKey().$default(() => ulid()),
...
export const productOptions = sqliteTable('product_options', {
  ...
  unique().on(table.productId, table.optionName),
]);
...
export const productOptionValues = sqliteTable('product_option_values', {
  id: text('id', { length: 26 }).primaryKey().$default(() => ulid()),
  productOptionId: text('product_option_id').notNull().references(() => productOptions.id, { onDelete: 'cascade' }),
...
export const insertProductVariantSchema = createInsertSchema(productVariants, {
  ...
  imageUrl: (schema) => schema.max(500).url().nullable().optional(),
...
export const productVariantOptions = sqliteTable('product_variant_options', {
  ...
  unique().on(table.productVariantId, table.optionName),
]);
